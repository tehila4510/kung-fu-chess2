...
set ENGINE_TEST_SOURCES=tests\test_main.cpp tests\RuleEngineTest.cpp tests\RealTimeArbiterTest.cpp tests\GameEngineTest.cpp tests\BoardMapperTest.cpp tests\ControllerTest.cpp tests\BoardIOTest.cpp tests\GraphicsBoardLayoutTest.cpp tests\GraphicsConfigTest.cpp tests\GraphicsAssetPathsTest.cpp tests\MatchQueueTest.cpp tests\ConnectionRegistryTest.cpp tests\MatchCoordinatorTest.cpp src\server\MatchQueue.cpp src\server\ConnectionRegistry.cpp src\server\MatchCoordinator.cpp %ENGINE_SOURCES%
...
if /i "%~1"=="test" (
    "%GPP%" %COMMON_FLAGS% -Ithird_party\websocketpp %ENGINE_TEST_SOURCES% %GRAPHICS_STL_SOURCES% -o build\KungFuChessEngineTests.exe
    if errorlevel 1 exit /b 1
